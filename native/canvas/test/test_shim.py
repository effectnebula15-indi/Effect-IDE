...
def main():
    ...
    # Нижняя граница — то, ради чего проверка существует: без сна цикл крутится
    # на полной скорости, и это видно сразу.
    assert elapsed >= 5 / 60 * 0.9, "ограничитель не удерживает частоту: %.4f с" % elapsed

    # Верхняя нарочно широкая. Узкая проверяет не наш код, а загруженность
    # машины: на общем раннере CI time.sleep регулярно перелетает в разы, и
    # граница в три раза уже роняла сборку на macOS. Здесь ловится только
