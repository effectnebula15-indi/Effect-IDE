...
/**
 * Шаг размера шрифта для команд «увеличить» и «уменьшить».
 *
 * Заметный, но не прыжок через размер. Здесь, а не в точках сборки: команда
 * одна и та же на телефоне и на десктопе, а два числа в двух местах разъезжаются
 * молча.
 */
const val FONT_STEP_SP = 2f

/**
 * Подрезает размер шрифта по тем же границам, что и жест двумя пальцами.
 *
 * Публичная затем, что менять размер умеет не только жест: то же делают команды
 * палитры, а завтра — настройки. Границы обязаны быть одни: разъехавшиеся копии
 * одного правила в этом проекте уже давали курсор не там, куда ткнули.
 */
fun clampFontSize(sizeSp: Float): Float = sizeSp.coerceIn(MIN_FONT_SIZE_SP, MAX_FONT_SIZE_SP)
